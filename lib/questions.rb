...
def select_elements_starting_with_vowel(array)
  array.delete_if { |x| !x.start_with?('a', 'e', 'i', 'o', 'u') }
end
...
def number_of_elements_that_are_palindromes(array)
  count = 0
  array.each { |x| x == x.reverse ? count += 1 : nil }
  count
end
...
def get_elements_until_greater_than_five(array)
  new_array = Array.new
  array.each { |x| x > 5 ? break : new_array.push(x) }
  new_array
end
...
def remove_capital_letters_from_string(string)
  letters = string.split('')
  letters.delete_if { |x| x == x.upcase && x != ' ' }
  letters.join('')
end
...
def is_a_2014_bank_holiday?(date)
  ...
  Time.new(2014, 4, 21),Time.new(2014, 5, 5),Time.new(2014, 5, 26),
Time.new(2014, 8, 25),Time.new(2014, 12, 25),Time.new(2014, 12, 26)]
...
def fizzbuzz_without_modulo
  ...
  for x in 1..100
    if fifteens.include?(x)
      puts "FizzBuzz"
    elsif fives.include?(x)
      puts "Buzz"
    elsif threes.include?(x)
      puts "Fizz"
    else
      puts x
    end
